...
public class CodeMirrorEditor implements CodeEditor {
    ...
    public void init(Runnable... runAfterLoading) {
        try {
            final URI editorUri = CodeMirrorEditor.class.getResource("/html/editor.html").toURI();
    ...
    public void autoFormat() {
        Platform.runLater(() -> {
            webView.getEngine().executeScript("autoFormat();");
        });
    }
    ...
    @Override
    public void setJsonLint(boolean lint) {
        Platform.runLater(() -> {
            webView.getEngine().executeScript("codeMirror.setOption('lint', " + lint + ");");
        });
    }
